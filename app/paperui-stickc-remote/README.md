# paperui-stickc-remote

M5StickC Plus2 firmware: press **Button A** to transmit the Electrolux AC **OFF** frame
over the built-in IR LED (GPIO19). Built on the PaperUI Rust stack (engine + widgets +
DefaultTheme + embedded-graphics adapter + the `paperui-tft` esp-hal backend).

## Build
```bash
source ~/export-esp.sh
cargo +esp build -p paperui-stickc-remote -Zbuild-std=core \
  --target xtensa-esp32-none-elf --release
```
Produces `target/xtensa-esp32-none-elf/release/paperui-stickc-remote` (a valid Xtensa ELF).
...
espflash flash --monitor \
  target/xtensa-esp32-none-elf/release/paperui-stickc-remote
```
