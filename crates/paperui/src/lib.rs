...
//!
//! One crate holding the engine (the `Canvas`/`Widget`/`Theme`/`Renderer`/
//! `InputSource` traits, geometry, `State`, button gestures), the pure-logic
//! widgets, and the default themes. It depends on **no graphics library** by
//! default; the `embedded-graphics` adapter is behind the `eg` feature, and a
//! recording mock `Canvas` for host tests is behind `mock`.
//!
...
mod types;
mod widget;
// --- widgets + themes (always on; pure logic, no extra deps) ---
mod button;
mod theme;
mod widget_theme;
...
pub use theme::{DefaultTheme, EinkTheme};
